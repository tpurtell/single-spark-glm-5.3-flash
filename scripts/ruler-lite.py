#!/usr/bin/env python3
"""RULER-lite: synthetic long-context eval for GLM single-Spark serving.
...
CWE_WORDS = ["apple", "banana", "cherry", "dragon", "eagle", "forest", "garden",
             ...
             "granite", "ivory", "jasmine", "kingdom", "lagoon", "mirror",
...
def chat(base_url: str, model: str, prompt: str, max_tokens: int = 512,
         temperature: float = 0.0, thinking_key: str = "thinking", token_ids=None) -> dict:
    if thinking_key == 'glm-closed-think':
        tokens = token_ids if token_ids is not None else visible_prompt(
            base_url, model, [{'role': 'user', 'content': prompt}], REQUEST_TIMEOUT)
        t0 = time.perf_counter()
        result = request_json(base_url + '/completions', {
            'model': model, 'prompt': tokens, 'add_special_tokens': False,
            'max_tokens': max_tokens, 'temperature': temperature, 'seed': 42,
            'cache_salt': hashlib.sha256(prompt.encode()).hexdigest(),
        })
        return {'prediction': (result['choices'][0].get('text') or '').strip(),
                'seconds': time.perf_counter() - t0, 'response': result,
                'finish_reason': result['choices'][0].get('finish_reason'),
                'usage': result.get('usage') or {}, 'rendered_tokens': len(tokens)}
    body = {"model": model,
    ...
    return {'prediction': (d['choices'][0]['message'].get('content') or '').strip(),
            'seconds': wall, 'response': d, 'finish_reason': d['choices'][0].get('finish_reason'),
            'usage': d.get('usage') or {}, 'rendered_tokens': None}


def exact_visible_case(base_url: str, model: str, prompt: str, answer_prefix: str,
                       target: int, render_fn=None) -> tuple[str, list[int]]:
    """Keep task facts intact and the answer prefix last; count chat framing."""
    render = render_fn or (lambda text: visible_prompt(
        base_url, model, [{'role': 'user', 'content': text}], REQUEST_TIMEOUT))
    prefix = prompt + '\n\n'
    suffix = '\n' + answer_prefix
    unit = ' ' + HAYSTACK_SENTENCE
    fixed = len(render(prefix + suffix))
    unit_cost = len(render(prefix + unit + suffix)) - fixed
    if target <= fixed + 128 or unit_cost <= 0:
        raise ValueError('target is too small for this task and framing')
    copies = max(0, (target - fixed - 128) // unit_cost)
    head = prefix + unit * copies
    padding = target - len(render(head + suffix))
    for _ in range(8):
        if padding < 0:
            raise RuntimeError('task padding exhausted its exact-length reserve')
        text = head + ' x' * padding + suffix
        tokens = render(text)
        if len(tokens) == target:
            return text, tokens
        padding += target - len(tokens)
    raise RuntimeError('could not produce exact rendered RULER-lite token count')
...
def run_case(base_url: str, model: str, length: int, make_task, rng: random.Random,
    ...
    prompt, golds, answer_prefix, task = make_task(rng)
    token_ids = None
    if thinking_key == 'glm-closed-think':
        padded, token_ids = exact_visible_case(base_url, model, prompt, answer_prefix, length)
    ...
    response = chat(base_url, model, padded, thinking_key=thinking_key,
                    max_tokens=max_tokens, token_ids=token_ids)
    pred = response['prediction']
    semantic_match = score_answer(pred, golds, task)
    exact_usage = response['usage'].get('prompt_tokens') == actual
    ok = semantic_match and response['finish_reason'] == 'stop' and (
        exact_usage or thinking_key != 'glm-closed-think')
    return {"task": task, "target": length, "actual_tokens": actual,
            "ok": ok, 'semantic_match': semantic_match, 'exact_prompt_usage': exact_usage,
            "prediction": pred, "gold": golds, "secs": round(response['seconds'], 1),
            'finish_reason': response['finish_reason'], 'usage': response['usage'],
            'response': response['response'], 'prompt': padded,
            'prompt_sha256': hashlib.sha256(padded.encode()).hexdigest()}
...
def main() -> int:
    ...
    ap.add_argument("--model", default="vcruz305/GLM-5.3-Flash-EXL3-K2")
    ...
    ap.add_argument("--thinking-key", default="glm-closed-think",
    ...
    report = {'schema': 'glm53-ruler-lite.v2', "model": args.model,
              "base_url": args.base_url, "seed": args.seed,
              'generation_seed': 42, 'max_tokens': args.max_tokens,
              'framing': args.thinking_key,
              'scope': 'Synthetic RULER-like early-fact tasks, not official RULER; '
                       'gold inclusion plus clean stop; distributed-depth retrieval is separate',
              "cases": []}
    failures = 0
    for length in lengths:
        for make_task in tasks:
            ...
            if not case.get("ok"):
                failures += 1
            detail = case.get("prediction", case.get("error", ""))[:160]
